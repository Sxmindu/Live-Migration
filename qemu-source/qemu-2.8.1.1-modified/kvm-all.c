...
// Jon: set DEBUG_FT_LVL
#define DEBUG_FT_LVL 2
...
//Modified to get dirty bitmap of destination - postcopy_FT
static int kvm_get_dirty_pages_log_range_new(MemoryRegionSection *section,
                                         unsigned long *bitmap)
{
    ram_addr_t start = section->offset_within_region +
                       memory_region_get_ram_addr(section->mr);
    ...
    int count = 0;
	count = cpu_physical_memory_set_dirty_lebitmap_new(bitmap, start, pages);
    return count;
}
                                          ...
                                          MemoryRegionSection *section)
{
    ...
    while (start_addr < end_addr) {
		...
		if(FT_tracker == 1)
		{
			count_A += kvm_get_dirty_pages_log_range_new(section, d.dirty_bitmap);
		}
		else
		{		
			kvm_get_dirty_pages_log_range(section, d.dirty_bitmap);
		}
		//count = ctpopl(d.dirty_bitmap);
		//printf("%s : count %ld \n",__func__,count);

        start_addr = mem->start_addr + mem->memory_size;
    }
	//printf("%s : FF count %ld \n",__func__,count);

...
/*	char **ips = get_packet_ips(iov,iov_cnt);
	char *src = ips[0];
	char *dst = ips[1];
...
static ssize_t virtio_net_receive(NetClientState *nc, const uint8_t *buf, size_t size)
{
    ...
    char **ips = get_packet_ips(buf,size);
    char *src = ips[0];
    char *dst = ips[1];
    printf("%s : src <%s> dst <%s> : addr %ld \n", __func__, src, dst,buf);
...
static int32_t virtio_net_flush_tx(VirtIONetQueue *q)
{
    ...
    for (;;) {
		...
		char **ips = get_packet_ips(out_sg->iov_base,out_sg->iov_len);
		char *src = ips[0];
		char *dst = ips[1];
		printf("%s : src <%s> dst <%s> : addr %ld \n", __func__, src, dst,out_sg->iov_base);

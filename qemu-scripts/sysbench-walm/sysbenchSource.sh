...
echo $pid
top -b -d 0.05 -n 500 -p "$pid" | grep --line-buffered 'sysbench' | ts %M:%.S | sed -n '1, 500{s/^ *//;s/ *$//;s/  */,/gp;};500q' >> /home/base/Desktop/cpu_usage.log &
EOF

if [ "$TYPE" = "tcp" ]
then
	echo "Starting Precopy Migration" | ts
	cat ../migration/precopy/precopy-vm-migrate.txt | sudo socat - /media/qmp1
elif [ "$TYPE" = "pp" ]
then
	echo "Starting Postcopy Migration" | ts
...
then
	echo "Starting Hybrid Migration" | ts
